      ...
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-col lg:flex-row items-end justify-between gap-12 mb-20">
          <div className="lg:w-2/3">
            ...
            <p className="text-slate-700 text-xl leading-relaxed max-w-2xl font-medium">
              Area Banaran dan Muara Progo merupakan salah satu jalur singgah (stopover) krusial bagi berbagai jenis burung migran dari belahan bumi utara sebelum melanjutkan perjalanan ke Australia. Selain itu, area ini juga merupakan tempat tinggal berbagai jenis burung endemik dari Indonesia.
            </p>
          </div>
          <div className="hidden lg:block">
            <div className="w-32 h-32 border-2 border-sky-200 rounded-full flex items-center justify-center animate-spin-slow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-32">
          {birds.map((bird) => (
            <div 
              key={bird.id} 
              onClick={() => setSelectedBird(bird)}
              className="group bg-white rounded-[2.5rem] overflow-hidden shadow-2xl shadow-sky-900/5 border border-sky-100 hover:border-sky-400 transition-all duration-700 hover:-translate-y-4 cursor-pointer flex flex-col h-full"
            >
              <div className="h-64 overflow-hidden relative">
                <img src={bird.image} alt={bird.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                {bird.status && (
                  <div className="absolute bottom-4 left-4 flex flex-wrap gap-1">
                    {bird.status.split(' | ').map((s, i) => (
                      <span key={i} className="bg-white/90 backdrop-blur-md text-sky-900 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm">
                        {s}
                      </span>
                    ))}
                  </div>
              ...
              <div className="p-8 flex flex-col flex-1">
                <h4 className="text-xl font-bold text-slate-900 mb-1">{bird.name}</h4>
                <p className="text-sky-600 text-[10px] italic font-black uppercase tracking-widest mb-4">{bird.scientific}</p>
                <p className="text-slate-600 text-sm leading-relaxed line-clamp-3 font-medium mb-6 flex-1">{bird.description}</p>
