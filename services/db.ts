...
const isConfigured = SUPABASE_URL.startsWith('http') && SUPABASE_URL.length > 10;
...
export const db = {
  ...
  syncLocalToCloud: async () => {
    if (!supabase) return { success: false, message: 'Supabase belum dikonfigurasi di Vercel.' };
    try {
      const attractions = getLocal('attractions') || [];
      const umkm = getLocal('umkm') || [];
      const birds = getLocal('birds') || [];
      ...
      return { success: true, message: 'Data berhasil diunggah ke Cloud!' };
    } catch (error: any) {
      console.error('Sync Error:', error);
      return { success: false, message: error.message };
    }
